...
C  ccdetect
C
C Cross-Correlation Detector (primitive!)
C
...
C WF  templateSACFILEname      target2SACFILEname  - for a waveform, or
C FS  lofreq  hifreq   iord    passes  - for a filter specification
C
C
      PROGRAM ccdetect
      IMPLICIT NONE
...
c     CHARG  = ' '
c     IARG   = 1
c     CALL GETARG( IARG, CHARG )
c     EV1CODE = CHARG(1:20)
C
C Read EV2CODE
C
c     CHARG  = ' '
...
c     IARG   = 5
c     CALL GETARG( IARG, CHARG )
c     READ ( CHARG, *, END=99, ERR=99 ) DTXTEM
C
C Read DTXTAR
C
c     CHARG  = ' '
c     IARG   = 6
c     CALL GETARG( IARG, CHARG )
c     READ ( CHARG, *, END=99, ERR=99 ) DTXTAR
C
C Read DTMPWN
C
c     CHARG  = ' '
c     IARG   = 7
c     CALL GETARG( IARG, CHARG )
c     READ ( CHARG, *, END=99, ERR=99 ) DTMPWN
C
C Read DTARWN
C
c     CHARG  = ' '
c     IARG   = 8
c     CALL GETARG( IARG, CHARG )
c     READ ( CHARG, *, END=99, ERR=99 ) DTARWN
C
C Read ICC
C
      CHARG  = ' '
      ...
      ENDIF
C
c     WRITE (6,*) 'EV1CODE ', EV1CODE
c     WRITE (6,*) 'EV2CODE ', EV2CODE
c     WRITE (6,*) 'STACODE ', STACODE
c     WRITE (6,*) 'PHACODE ', PHACODE
c     WRITE (6,*) 'DTXTEM  ', DTXTEM 
c     WRITE (6,*) 'DTXTAR  ', DTXTAR 
c     WRITE (6,*) 'DTMPWN  ', DTMPWN 
c     WRITE (6,*) 'DTARWN  ', DTARWN 
      WRITE (6,*) 'ICC     ', ICC    
C 
...
C
c     NB4TEMREF = INT( DTXTEM*DINTAT )
      NB4TEMREF = 0
c     DTEMRF    = DTIM0A + DBLE( NB4TEMREF )*DELTAT
      DTEMRF    = DTIM0A 
c     DTEMSTART = DTEMRF - DBLE( NTMPWN    )*DELTAT
      DTEMSTART = DTEMRF
c     NTEMSTART = NB4TEMREF - NTMPWN
      NTEMSTART = 0
...
C
c     NB4TARREF = INT( DTXTAR*DINTAT )
      NB4TARREF = 0
c     DTARRF    = DTIM0B + DBLE( NB4TARREF )*DELTAT
      DTARRF    = DTIM0B
c     DTARSTART = DTARRF - DBLE( NTARWN    )*DELTAT
      DTARSTART = DTARRF
c     NTARSTART = NB4TARREF - NTARWN
      NTARSTART = 0
C
...
C       .
        ...
        IN1TAR = 1
        IN1COR = 1
c           NTSTAR = 2*NTARWN + NLENWN - (ICORR-1)*NWSTEP
c       NTSTAR = NTARWN
        NELCC  = NTARWN - NTMPWN + 1
C           .
